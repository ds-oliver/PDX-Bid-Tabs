...
def test_pay_item_similarity_workbook_structure_and_unclassified():
    ...
    expected = ["Spec Code", "Project Count", "Distinct Descriptions", "Sample Descriptions", "Variance Level"]
    got = [ws.cell(1, i).value for i in range(1, len(expected) + 1)]
    assert got == expected

    ws_high = wb["HIGH_VARIANCE_DETAILS"]
    expected_high = ["Spec Code", "Clean Description", "Original Raw Text", "Project EAN", "Avg Unit Price", "Reviewer Notes"]
    ...
    assert "UNCLASSIFIED" in set(summary["Spec Code"].astype(str))
    row = summary[summary["Spec Code"] == "UNCLASSIFIED"].iloc[0]
