...
def main():
    ...
    if not line_path.exists() or not bidder_path.exists():
        raise SystemExit("Required inputs missing: fact_bid_item_price.csv, dim_bidder.csv")

    line = pd.read_csv(line_path, keep_default_na=False)
    bidder = pd.read_csv(bidder_path, keep_default_na=False)

    line = _to_num(line, ["unit_price"])
    line = line.merge(bidder[["bidder_id", "bidder_type"]], on="bidder_id", how="left")
    contractors = line[(line["bidder_type"] == "CONTRACTOR") & (line["unit_price"].notna())].copy()

    group_keys = ["project_id", "bid_schedule_id", "pay_item_id"]
    metrics = contractors.groupby(group_keys, as_index=False).agg(
    ...
    metrics.to_csv(output_dir / "fact_project_pay_item_metrics.csv", index=False)
